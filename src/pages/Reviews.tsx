...
export default function Reviews() {
  ...
  const allRatings = [...googleReviews.map(r => r.rating), ...userReviews.map(r => r.rating)];
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-start">
                    ...
                    <span className="text-sm text-muted-foreground">
                      Based on {googleReviews.length + userReviews.length} reviews
                    </span>
                  </div>
                </div>
        ...
        <section className="section bg-card">
          <div className="container">
            <div className="max-w-2xl mx-auto">
              <ReviewForm onReviewSubmitted={fetchUserReviews} />
            </div>
          </div>
        </section>
          ...
          <div className="container">
            <Tabs defaultValue="all" className="w-full">
              <div className="flex justify-center mb-8">
                <TabsList>
                  <TabsTrigger value="all">All Reviews ({googleReviews.length + userReviews.length})</TabsTrigger>
                  <TabsTrigger value="website">Website Reviews ({userReviews.length})</TabsTrigger>
                  <TabsTrigger value="google">Google Reviews ({googleReviews.length})</TabsTrigger>
                </TabsList>
              </div>
              ...
              <TabsContent value="all">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  {userReviews.map((review, index) => (
                    <Card
                      key={review.id}
                      className="animate-fade-in hover:shadow-lg transition-shadow border-primary/20"
                  ...
                  {googleReviews.map((review, index) => (
                    <Card
                      ...
                      style={{ animationDelay: `${(userReviews.length + index + 1) * 50}ms` }}
                    >
                      <CardContent className="p-6">
                        <div className="flex items-center gap-4 mb-4">
                          <Avatar className="h-12 w-12">
